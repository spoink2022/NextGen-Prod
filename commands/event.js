...
async function sendPick(msg) {
    ...
    msg.channel.send(embed).then(async(sentMessage) => {
        ...
        if(chartData[0].t < new Date(quote.latestUpdate)) { chartData.unshift({t: new Date(quote.latestUpdate), y: quote.price}); }
        ...
        if(chartData2[0].t < new Date(quote2.latestUpdate)) { chartData2.unshift({t: new Date(quote2.latestUpdate), y: quote2.price}); }
