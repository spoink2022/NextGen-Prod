...
async function onGuildMemberAdd(guildMember) {
    if(guildMember.user.bot || config.prefix === 'ng!') { return; }
    ...
    if(userIsNew) {
        guildMember.createDM().then((dmChannel) => {
            dmChannel.send(`Hey there, welcome to the server!
To get started with investing, just type \`${config.prefix}init\` either here or in the server.
Oh, and also, don't forget to check out server-info for info on all the channels!`);
